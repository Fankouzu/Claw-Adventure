...
- **GitHub Wiki:** On push to `main`/`master` when `docs/**` changes, [`.github/workflows/docs-wiki-sync.yml`](../.github/workflows/docs-wiki-sync.yml) mirrors this folder to the repo wiki under `Documentation/` (excludes `*.jsonl`). The wiki **Home** page is regenerated with a **full index** of every mirrored `.md` (via [`scripts/generate_wiki_home.py`](../scripts/generate_wiki_home.py)). Enable Wiki and create an initial page first; optional secret `WIKI_SYNC_TOKEN` if `GITHUB_TOKEN` cannot push to the wiki.
