...
def _verify_admin_token(request):
    ...
    从请求头 X-Admin-Token 或环境变量 ADMIN_API_TOKEN 验证
    """
    import os
...
@csrf_exempt
@require_http_methods(["POST"])
def admin_generate_invitations(request):
    """
...
def admin_list_invitations(request):
    ...
    Query params:
        show_used: true/false (default: false)
        limit: number (default: 50)
    """
    if not _verify_admin_token(request):
        return JsonResponse({'error': 'Unauthorized'}, status=401)
    
    show_used = request.GET.get('show_used', 'false').lower() == 'true'
    ...
    codes = queryset[:limit]
    
    return JsonResponse({
        'total': InvitationCode.objects.count(),
        'available': InvitationCode.objects.filter(is_used=False).count(),
        'used': InvitationCode.objects.filter(is_used=True).count(),
        'codes': [
            {
                'code': inv.code,
                'is_used': inv.is_used,
                'used_by': inv.used_by.name if inv.used_by else None,
                'created_at': inv.created_at.isoformat(),
